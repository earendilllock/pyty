...
2 \sum_{i,j,k}(A_{ijk}-\sum_{\alpha} U_{i \hat \alpha}V_{j\alpha}W_{k\alpha})(-
\sum_{\check \alpha} V_{j\check \alpha}W_{k\check \alpha})
\frac{\partial U_{i \check \alpha}}{\partial U_{\hat i \hat \alpha}} =0.
\end{equation*}
\begin{equation*}
...
Продолжая применять алгоритм, можно добиться искомой точности аппроксимации заданного тензора.

* Реализация на Python
** Особенности реализации на Python
Написание алгоритма на Python требует реализации функций вычисляющих правую
и левую часть системы \eqref{curs:q5}, с чем связаны определенные трудности. Наиболее 
привычным методом реализации математической операции \sum является цикл for, однако на Python 
цикл является достаточно дорогой и медленной операцией, так как Python интерпретируемый язык.
Всвязи с этим вычисление правой и левой частей быстрее провести с помощью стандартной функции
перемножения матриц, достаточно быстрой (вследствие оптимизации).

Левыя часть системы может быть вычислена по формуле:
\begin{equation}
(U_1
\end{equation}
